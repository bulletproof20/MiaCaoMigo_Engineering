...
|-------------|------------|------------------|----------------------|
| APP-EV-M1-01 | RF_M1_01 | `POST /api/users/auth/register`, `FrontEnd/Pages/Mod1_Users/Autenticacao/criar_conta.html`, `FrontEnd/Js/Mod1/criarConta.js` | Client account registration exposed through UI and API |
...
| APP-EV-M1-04 | RF_M1_39, RF_M1_41 | `GET /api/users/auth/setup`, `PUT /api/users/auth/setup/theme`, `setup.the_set`, `Sidebar/ClientSidebar.js`, `authSession.js` | Client light/dark theme is read from and persisted to the database without keeping preference history |
| APP-EV-M1-05 | RF_M1_33–37, RF_M1_39 | `GET /api/users/staff/me/agenda`, `staff.js`, `MainDashboard.html`, `staffDashboardHome.js`, `staffArea.js` | Staff self-service dashboard with role metadata (veterinarian/assistant/profiles) and personal operational widgets; full agenda JS/API exists, but `AreaFuncionario.html` currently has an empty shell |
| APP-EV-M1-06 | RF_M1 staff management | `POST /api/users/employees`, `FrontEnd/Pages/Mod1_Users/Funcionarios/AdicionarFuncionario.html`, `FrontEnd/Js/Mod1/AdicionarFuncionario.js` | Staff onboarding protected by `manage_employees`; creates identity + employee and returns generated work email |
| APP-EV-M1-07 | RF_M1 staff management | `SidebarMenuCatalog.js`, `EmployeeSidebar.js`, `QuadroFuncionario.html`, `CalendarioEquipa.html`, `Funcionarios/Views/*.html` | Role-aware staff navigation and HR presentation/prototype views for employee management |
